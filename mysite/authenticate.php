...
$stmt = $con->prepare('SELECT id, firstname, password, rememberme, activation_code, role, profit, registered_date, amount_invested, earnings_mon  FROM accounts WHERE username = ?');
...
if ($stmt->num_rows > 0) {
	$stmt->bind_result($id, $firstname, $password, $rememberme, $activation_code, $role, $profit, $registered_date, $amount_invested, $earnings_mon);
	$stmt->fetch();
	$stmt->close();
